...
# We want to send our registration data to the label server.
#
...
cat json.dat
curl -s -f -X PUT -d @json.dat http://moorescloud.local:2343/firstboot/register
if [ $? == 0 ]
then
	echo ":: Registration data successfully transmitted"
	rm json.dat
	exit 0
else
	echo ":: Registration data tranmission FAILURE"
